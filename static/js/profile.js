...
async function generateReport(taskId, task) {
    ...
    // --- .stat данные ---
    let skyPlotSvg = '', phaseResSvg = '', rangeResSvg = '', tropoSvg = '', clockSvg = '';
    try {
        const sr = await fetch(`/api/measurements/stats?id=${taskId}`, {
            headers: { 'Authorization': `Bearer ${getToken()}` }
        });
        if (sr.ok) {
            const sd = await sr.json();
            if (sd.satellites?.length)  skyPlotSvg  = buildSkyPlotSvg(sd.satellites);
            if (sd.satellites?.length)  phaseResSvg = buildResidualsSvg(sd.satellites, 'resc', 'Остатки фазовых измерений, м', 0.008);
            if (sd.satellites?.length)  rangeResSvg = buildResidualsSvg(sd.satellites, 'resp', 'Остатки псевдодальностей, м', 2.5);
            if (sd.tropo?.length)       tropoSvg    = buildLineChartSvg(sd.tropo, 'trp', 'Тропосферная задержка, м');
            if (sd.clock?.length)       clockSvg    = buildLineChartSvg(sd.clock, 'bias', 'Смещение часов приёмника, нс');
        }
    } catch(e) { /* нет данных — секция не отображается */ }
...
${qTimelineSvg ? `<p style="font-size:9pt;color:#555;margin-bottom:4px;">Качество решения по эпохам</p>
<div style="margin-bottom:14px;">${qTimelineSvg}</div>` : ''}
<div style="display:flex;gap:20px;align-items:flex-start;flex-wrap:wrap;margin-bottom:10px;">
...
${(skyPlotSvg || phaseResSvg || rangeResSvg || tropoSvg || clockSvg) ? `
<h2>Данные обработки RTKLIB</h2>
<div style="display:flex;gap:20px;align-items:flex-start;flex-wrap:wrap;margin-bottom:14px;">
${skyPlotSvg  ? `<div><p style="font-size:9pt;color:#555;margin-bottom:4px;">Небосвод спутников</p>${skyPlotSvg}</div>` : ''}
${tropoSvg    ? `<div><p style="font-size:9pt;color:#555;margin-bottom:4px;">Тропосферная задержка</p>${tropoSvg}</div>` : ''}
${clockSvg    ? `<div><p style="font-size:9pt;color:#555;margin-bottom:4px;">Смещение часов</p>${clockSvg}</div>` : ''}
</div>
${phaseResSvg ? `<p style="font-size:9pt;color:#555;margin-bottom:4px;">Остатки фазовых измерений, м</p><div style="margin-bottom:14px;">${phaseResSvg}</div>` : ''}
${rangeResSvg ? `<p style="font-size:9pt;color:#555;margin-bottom:4px;">Остатки псевдодальностей, м</p><div style="margin-bottom:14px;">${rangeResSvg}</div>` : ''}
` : ''}
...
function buildTrajectorySvg(td) {
    ...
    for (let i = 0; i < pts.length; i += step) {
        const p = pts[i];
        circles += `<circle cx="${px(p).toFixed(1)}" cy="${py(p).toFixed(1)}" r="2" fill="${qColor(p.q)}"/>`;
    }

    // Стрелка направления в конце трека
    let arrow = '';
    if (pts.length >= 2) {
    ...
    const scaleBar = `<text x="${PAD}" y="${H-6}" font-size="8" fill="#999" font-family="monospace">
        ΔB=${mLat}м  ΔL=${mLon}м  n=${pts.length} эп.</text>`;

    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
        <rect width="${W}" height="${H}" fill="#fafafa"/>
        <path d="${path}" fill="none" stroke="#ccc" stroke-width="1"/>
        ${circles}
        ${arrow}
        ${scaleBar}
    </svg>`;
...
// ── Столбчатая диаграмма FIX / FLOAT ───────────────────────────────────────
...
// ── Цвет спутника по системе ────────────────────────────────────────────────
function satColor(sat) {
    if (sat.startsWith('G')) return '#3b82f6'; // GPS — синий
    if (sat.startsWith('R')) return '#ef4444'; // GLONASS — красный
    if (sat.startsWith('E')) return '#22c55e'; // Galileo — зелёный
    if (sat.startsWith('C')) return '#f59e0b'; // BeiDou — оранжевый
    if (sat.startsWith('J')) return '#a855f7'; // QZSS — фиолетовый
    return '#6b7280';
}

// ── Sky Plot (небосвод спутников) ────────────────────────────────────────────
function buildSkyPlotSvg(sats) {
    const S = 260, cx = S/2, cy = S/2, R = S/2 - 20;

    // Концентрические окружности и метки угла места
    let rings = '';
    [0,30,60,90].forEach(el => {
        const r = R * (90 - el) / 90;
        rings += `<circle cx="${cx}" cy="${cy}" r="${r.toFixed(1)}" fill="none" stroke="#e5e7eb" stroke-width="0.6"/>`;
        if (el > 0) rings += `<text x="${(cx+r+3).toFixed(1)}" y="${(cy+3).toFixed(1)}" font-size="8" fill="#bbb" font-family="monospace">${el}°</text>`;
    });

    // Перекрестье N/S/E/W
    const cross = `
        <line x1="${cx}" y1="${cy-R-10}" x2="${cx}" y2="${cy+R+10}" stroke="#e5e7eb" stroke-width="0.5"/>
        <line x1="${cx-R-10}" y1="${cy}" x2="${cx+R+10}" y2="${cy}" stroke="#e5e7eb" stroke-width="0.5"/>
        <text x="${cx}" y="${cy-R-12}" font-size="9" fill="#aaa" font-family="monospace" text-anchor="middle">С</text>
        <text x="${cx}" y="${cy+R+20}" font-size="9" fill="#aaa" font-family="monospace" text-anchor="middle">Ю</text>
        <text x="${cx+R+14}" y="${cy+4}" font-size="9" fill="#aaa" font-family="monospace" text-anchor="middle">В</text>
        <text x="${cx-R-14}" y="${cy+4}" font-size="9" fill="#aaa" font-family="monospace" text-anchor="middle">З</text>`;

    // Группируем по спутнику — рисуем трек
    const bySat = {};
    sats.forEach(s => {
        if (!bySat[s.sat]) bySat[s.sat] = [];
        bySat[s.sat].push(s);
    });

    let tracks = '', dots = '';
    Object.entries(bySat).forEach(([sat, pts]) => {
        const col = satColor(sat);
        pts.sort((a,b) => a.tow - b.tow);
        const coords = pts.map(p => {
            const azR = (p.az - 90) * Math.PI / 180; // 0°=N → SVG X
            const r   = R * (90 - p.el) / 90;
            return [cx + r * Math.cos(azR), cy + r * Math.sin(azR)];
        });
        if (coords.length > 1) {
            const d = coords.map((c,i) => `${i===0?'M':'L'}${c[0].toFixed(1)},${c[1].toFixed(1)}`).join(' ');
            tracks += `<path d="${d}" fill="none" stroke="${col}" stroke-width="1.2" opacity="0.5"/>`;
        }
        // Последняя позиция — точка с меткой
        const last = coords[coords.length - 1];
        dots += `<circle cx="${last[0].toFixed(1)}" cy="${last[1].toFixed(1)}" r="3.5" fill="${col}" opacity="0.85"/>`;
        dots += `<text x="${(last[0]+5).toFixed(1)}" y="${(last[1]+3).toFixed(1)}" font-size="7.5" fill="${col}" font-family="monospace" font-weight="bold">${sat}</text>`;
    });

    // Легенда систем
    const systems = [...new Set(sats.map(s => s.sat[0]))];
    const sysNames = {G:'GPS',R:'GLONASS',E:'Galileo',C:'BeiDou',J:'QZSS'};
    let legend = '';
    systems.forEach((sys, i) => {
        legend += `<circle cx="${10 + i*65}" cy="10" r="4" fill="${satColor(sys+'01')}"/>`;
        legend += `<text x="${18 + i*65}" y="14" font-size="8.5" fill="#555" font-family="monospace">${sysNames[sys]||sys}</text>`;
    });

    return `<svg width="${S}" height="${S+22}" viewBox="0 0 ${S} ${S+22}" xmlns="http://www.w3.org/2000/svg">
        <rect width="${S}" height="${S}" fill="#fafafa"/>
        ${rings}${cross}${tracks}${dots}
        <g transform="translate(0,${S+4})">${legend}</g>
    </svg>`;
}

// ── Остатки (phase / pseudorange residuals) ─────────────────────────────────
function buildResidualsSvg(sats, field, title, yLim) {
    const W = 700, H = 120, PAD_L = 56, PAD_B = 22, PAD_T = 10;
    const innerW = W - PAD_L - 10, innerH = H - PAD_B - PAD_T;

    const tows = sats.map(s => s.tow);
    const tMin = Math.min(...tows), tMax = Math.max(...tows) || tMin + 1;

    const px = t  => PAD_L + ((t - tMin) / (tMax - tMin)) * innerW;
    const py = v  => PAD_T + innerH/2 - (v / yLim) * (innerH/2);

    // Нулевая линия
    const zero = `<line x1="${PAD_L}" y1="${(PAD_T+innerH/2).toFixed(1)}" x2="${W-10}" y2="${(PAD_T+innerH/2).toFixed(1)}" stroke="#ddd" stroke-width="0.8"/>`;

    // Засечки Y
    let yAxis = '';
    [-yLim, -yLim/2, 0, yLim/2, yLim].forEach(v => {
        const y = py(v);
        yAxis += `<line x1="${PAD_L}" y1="${y.toFixed(1)}" x2="${W-10}" y2="${y.toFixed(1)}" stroke="#f0f0f0" stroke-width="0.5"/>`;
        yAxis += `<text x="${PAD_L-5}" y="${(y+3).toFixed(1)}" font-size="8.5" fill="#aaa" font-family="monospace" text-anchor="end">${v.toFixed(3)}</text>`;
    });
    ...
    sats.forEach(s => {
        const v = s[field];
        if (v === 0 && field === 'resc') return; // нет данных
        const x = px(s.tow), y = py(Math.max(-yLim, Math.min(yLim, v)));
        points += `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="1.5" fill="${satColor(s.sat)}" opacity="0.7"/>`;
    });

    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
        <rect width="${W}" height="${H}" fill="#fafafa"/>
        ${yAxis}${zero}
        <line x1="${PAD_L}" y1="${PAD_T}" x2="${PAD_L}" y2="${PAD_T+innerH}" stroke="#ccc" stroke-width="0.8"/>
        ${points}
        <text x="${PAD_L/2-4}" y="${PAD_T+innerH/2}" font-size="8.5" fill="#aaa" font-family="monospace"
              text-anchor="middle" transform="rotate(-90 ${PAD_L/2-4} ${PAD_T+innerH/2})">м</text>
    </svg>`;
}

// ── Линейный график (трopo / clock) ─────────────────────────────────────────
function buildLineChartSvg(data, field, label) {
    const W = 320, H = 120, PAD_L = 60, PAD_B = 22, PAD_T = 10;
    const innerW = W - PAD_L - 10, innerH = H - PAD_B - PAD_T;

    data.sort((a,b) => a.tow - b.tow);
    const vals = data.map(d => d[field]);
    const tows = data.map(d => d.tow);
    const vMin = Math.min(...vals), vMax = Math.max(...vals);
    const vRange = vMax - vMin || 0.001;
    const tMin = tows[0], tMax = tows[tows.length-1] || tMin+1;

    const px = t => PAD_L + ((t - tMin) / (tMax - tMin)) * innerW;
    const py = v => PAD_T + innerH - ((v - vMin) / vRange) * innerH;

    // Линия
    let path = `M${px(tows[0]).toFixed(1)},${py(vals[0]).toFixed(1)}`;
    for (let i = 1; i < data.length; i++) path += ` L${px(tows[i]).toFixed(1)},${py(vals[i]).toFixed(1)}`;

    // Σ (std) — серые усы
    let stds = '';
    data.forEach(d => {
        if (!d.std || d.std <= 0) return;
        const x = px(d.tow), yc = py(d[field]);
        const ys = (d.std / vRange) * innerH;
        stds += `<line x1="${x.toFixed(1)}" y1="${(yc-ys).toFixed(1)}" x2="${x.toFixed(1)}" y2="${(yc+ys).toFixed(1)}" stroke="#d1d5db" stroke-width="0.6"/>`;
    });

    // Засечки Y (4 штуки)
    let yAxis = '';
    for (let t = 0; t <= 3; t++) {
        const v = vMin + (t/3) * vRange;
        const y = py(v);
        yAxis += `<line x1="${PAD_L}" y1="${y.toFixed(1)}" x2="${W-10}" y2="${y.toFixed(1)}" stroke="#e5e7eb" stroke-width="0.6"/>`;
        yAxis += `<text x="${PAD_L-5}" y="${(y+3).toFixed(1)}" font-size="8" fill="#aaa" font-family="monospace" text-anchor="end">${v.toFixed(4)}</text>`;
    }
    ...
    return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
        ...
        ${yAxis}
        <line x1="${PAD_L}" y1="${PAD_T}" x2="${PAD_L}" y2="${PAD_T+innerH}" stroke="#ccc" stroke-width="0.8"/>
        <line x1="${PAD_L}" y1="${PAD_T+innerH}" x2="${W-10}" y2="${PAD_T+innerH}" stroke="#ccc" stroke-width="0.8"/>
        ${stds}
        <path d="${path}" fill="none" stroke="#3b82f6" stroke-width="1.5"/>
        <text x="${PAD_L/2-4}" y="${PAD_T+innerH/2}" font-size="8" fill="#aaa" font-family="monospace"
              text-anchor="middle" transform="rotate(-90 ${PAD_L/2-4} ${PAD_T+innerH/2})">${label.split(',')[1]||''}</text>
    </svg>`;
}
